    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      <section className="grid gap-5 md:grid-cols-2">
        <div className="md:col-span-2">
          ...
          <input name="fullName" value={form.fullName} onChange={handleChange} className={inputClass} required placeholder="Full name as you want it on the certificate" />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="email" name="email" value={form.email} onChange={handleChange} className={inputClass} required placeholder="name@domain.com" />
        </div>
        <div>
          <label className="text-sm font-semibold">Mobile number *</label>
          <input name="mobileNumber" value={form.mobileNumber} onChange={handleChange} className={inputClass} required placeholder="WhatsApp preferred" />
        </div>
        <div>
      ...
      <section className="space-y-5">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          <div>
            ...
            <select name="professionalCategory" value={form.professionalCategory} onChange={handleChange} className={inputClass} required>
          ...
          {form.professionalCategory === "Other" && (
            <div className="md:col-span-2">
              ...
              <input name="otherProfessionalCategory" value={form.otherProfessionalCategory} onChange={handleChange} className={inputClass} />
            </div>
          )}
          <div className="md:col-span-2">
            <label className="text-sm font-semibold">Department / specialization *</label>
            <input name="departmentSpecialization" value={form.departmentSpecialization} onChange={handleChange} className={inputClass} required />
          </div>
          <div className="md:col-span-2">
            <label className="text-sm font-semibold">Institution / hospital / organization name *</label>
            <input name="organizationName" value={form.organizationName} onChange={handleChange} className={inputClass} required />
          </div>
          <div>
            <label className="text-sm font-semibold">Type of organization *</label>
            <select name="organizationType" value={form.organizationType} onChange={handleChange} className={inputClass} required>
          ...
          <div>
            ...
            <select name="yearsExperience" value={form.yearsExperience} onChange={handleChange} className={inputClass} required>
          ...
          {form.organizationType === "Other" && (
            <div className="md:col-span-2">
              ...
              <input name="otherOrganizationType" value={form.otherOrganizationType} onChange={handleChange} className={inputClass} />
            </div>
          )}
        </div>
      </section>
      ...
      <section className="space-y-5">
        ...
        <div>
          ...
          <textarea name="otherObjective" value={form.otherObjective} onChange={handleChange} className={inputClass} rows={4} placeholder="Add anything else you'd like to learn" />
        </div>
        <div>
          <label className="text-sm font-semibold">Interested in future workshops? *</label>
          <select name="futureWorkshopsInterest" value={form.futureWorkshopsInterest} onChange={handleChange} className={inputClass} required>
      ...
      <section className="space-y-4 rounded-[1.75rem] border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-slate-950/60">
        ...
        <label className="flex gap-3 text-sm text-slate-700 dark:text-slate-200">
          <input type="checkbox" name="consentCommunications" checked={form.consentCommunications} onChange={handleChange} className="mt-1 h-4 w-4 rounded border-slate-300 text-teal-600 focus:ring-teal-500" />
          <span>
        ...
        <label className="flex gap-3 text-sm text-slate-700 dark:text-slate-200">
          <input type="checkbox" name="informationAccurate" checked={form.informationAccurate} onChange={handleChange} className="mt-1 h-4 w-4 rounded border-slate-300 text-teal-600 focus:ring-teal-500" />
