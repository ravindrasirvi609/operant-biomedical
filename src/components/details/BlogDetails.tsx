...
import blog_details_content from "@/data/blog_data";

const BlogDetails = ({ id }: any) => {
  const blogIdToShow = id;
  const blogDetails = getBlogDetailsById(blogIdToShow);

  if (!blogDetails) {
    return <div>Blog not found</div>;
  }

  const { title, des_1, features, qute, writer, title_2, des_2, des_3, image } =
    blogDetails;

  function getBlogDetailsById(id: any) {
    return blog_details_content.find((blog) => blog.id == id);
  }
      ...
      <section>
        <div className="container">
          <div className="anim_div_ShowZoom">
            <div className="cs_portfolio_details"></div>
          </div>
        </div>
      </section>
        ...
        <div className="container">
          <div className="cs_portfolio_details">
            <div className="reveal">
              <Image src={image} alt="blog_details_banner" />
            </div>
            ...
            <div className="cs_blog_detail_text_2 anim_div_ShowDowns">
              <p
                className="cs_text_style_body"
                dangerouslySetInnerHTML={{ __html: des_2 }}
              ></p>
            </div>
          </div>
        </div>
        ...
        <div className="cs_img_box cs_style_1">
          <div className="cs_img_show">
            <div className="cs_img_1 reveal">
              <Image src={image} alt="blog_details_1" />
            </div>
            <div className="cs_img_2 reveal">
              <Image src={image} alt="blog_details_2" />
            </div>
          </div>
          <div className="cs_height_30 cs_height_lg_30"></div>
          <div className="cs_img_show">
            <div className="cs_img_3 reveal">
              <Image src={image} alt="blog_details_3" />
            </div>
            <div className="cs_img_4 reveal">
              <Image src={image} alt="blog_details_4" />
            </div>
          </div>
        </div>
